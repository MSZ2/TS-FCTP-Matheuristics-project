...
vektorizovano preko NumPy broadcast-a (umesto Python petlje
...
umesto Python petlje \texttt{for k in range(K)} koja za svakog kupca
...
\Return $best$ (ili $a$, ako oba pokušaja propadnu)
\end{algorithm}
...
najnižoj proceni troška. Nakon \textsc{Repair}-a i provere dopustivosti
...
NumPy vektorizovanim ekvivalentima (broadcast, boolean maske, fensi
indeksiranje), bez promene rezultata pretrage -- ista RCL/$\alpha$ logika
...
\begin{itemize}
  ...
  \item Funkcija \texttt{\_construct\_and\_improve(alpha)} u worker procesu
    radi construction $+$ local search i vraća samo lagan rezultat
    ($x$, $y$, cena, \texttt{open\_dc}, broj poziva funkcije cilja u tom
    zadatku) -- ne šalje nazad cost-matrice instance, koje su nepromenjene
...
\textit{Platforma za CPLEX}: računar \texttt{ttbg-103}, Linux 6.6.87.2-WSL2,
IBM ILOG CPLEX, jedan procesni tok.

\subsection{Podešavanja GRASP-a, platforma i metodologija izveštaja}

\textit{Platforma za GRASP}: računar \texttt{ttbg-103}, Linux
6.6.87.2-microsoft-standard-WSL2, x86\_64, 16 CPU jezgara, Python 3.12.3,
NumPy 2.4.4. Algoritam je izvršavan paralelno sa
$n_{\text{workers}} = \text{cpu\_count}()-1 = 15$ worker procesa.

Svaka instanca je izvršena $k=3$ puta (seme $42 + 100r$ za $r$-to
...
\begin{itemize}
  ...
  \item Ugradnja intenzivnijeg path relinking-a koji istražuje i
    ...
    presek;
  \item Uvođenje Lagranževe relaksacije za generisanje boljih donjih
    granica i vođenje pretrage;
  \item Hibridizacija sa CPLEX-om -- korišćenje GRASP rešenja kao
    \textit{warm start} za MIP solver.
\end{itemize}
