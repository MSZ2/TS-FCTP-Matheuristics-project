...
class Construction:
    ...
    def build(self):
        ...
        for k in np.argsort(-data.d):          # largest demand first
            need = int(data.d[k])
            while need > 0:
                cand = []
...
class LocalSearch:
    ...
    def _swap_open_closed(self, sol):
        data = self.data
        closed = [j for j in range(data.J) if j not in sol.open_dc]
        best_cost = sol.cost
        best_trial = None
            ...
            for k in range(data.K):
                if trial.y[j, k] > 0:
                    best_j = min(
                        alts,
                        key=lambda jj: (
                            data.c[jj, k] * trial.y[j, k] +
                            (data.g[jj, k] if trial.y[jj, k] == 0 else 0)
                        )
                    )
    ...
    def _add_dc(self, sol):
        ...
        for j_new in closed:
            ...
            moved = False
            for k in range(data.K):
                curr_j = next((j for j in range(data.J) if trial.y[j, k] > 0), None)
                if curr_j is None:
                    continue
                old_cost = data.c[curr_j, k] * data.d[k] + data.g[curr_j, k]
                new_cost = data.c[j_new,  k] * data.d[k] + data.g[j_new,  k]
                if new_cost < old_cost:
                    trial.y[j_new,  k] += trial.y[curr_j, k]
                    trial.y[curr_j, k] = 0
                    moved = True
            if moved and repair(trial, data):
                trial.compute_cost()
                if trial.cost < best_cost:
                    best_cost = trial.cost
                    best_trial = trial
        ...
        for k in range(data.K):
            for j_from in range(data.J):
                ...
                for j_to in range(data.J):
                    ...
                    if data.c[j_to, k] >= data.c[j_from, k]:
                        continue
    ...
    def _two_customer_swap(self, sol):
        ...
        asgn = [
            (k, next((j for j in range(data.J) if sol.y[j, k] > 0), -1))
            for k in range(data.K)
        ]

        for i1 in range(data.K):
            k1, j1 = asgn[i1]
            if j1 < 0:
                continue
            for i2 in range(i1 + 1, data.K):
                k2, j2 = asgn[i2]
                if j2 < 0 or j1 == j2:
                    continue
                # Variable-cost delta (ignore fixed cost g here as a quick filter)
                delta = (
                    (data.c[j2, k1] - data.c[j1, k1]) * sol.y[j1, k1] +
                    (data.c[j1, k2] - data.c[j2, k2]) * sol.y[j2, k2]
                )
                if delta >= 0:
                    continue
                trial = sol.copy()
...
class PathRelinking:
    ...
    def relink(self, a, b):
        ...
        for dc_set in [a.open_dc | b.open_dc, a.open_dc & b.open_dc]:
            ...
            for k in range(data.K):
                best_j = min(dc_set, key=lambda j: data.c[j, k] * data.d[k] + data.g[j, k])
                trial.y[best_j, k] = data.d[k]
            if not repair(trial, data):
                continue
            trial.compute_cost()
