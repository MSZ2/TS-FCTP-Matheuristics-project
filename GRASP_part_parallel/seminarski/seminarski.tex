...
proporcionalan količini koja se njome prevozi, i fiksni, koji se plaća jednom
ako se veza uopšte koristi, tj.\ ako je protok na njoj pozitivan. Cilj je
minimizacija ukupnog troška transporta uz poštovanje ograničenja kapaciteta
dobavljača i tačno ispunjenje potražnje svih kupaca. Odluka o tome koje veze
...
Formulacija je preuzeta iz rada Calvete, Galé, Iranzo i Toth \cite{calvete2018},
...
jednom ako se odgovarajuća veza koristi. Ograničenje \eqref{eq:sup} obezbeđuje
...
\begin{enumerate}
  \item \textbf{Faza konstrukcije}: gradi se dopustivo rešenje
    ...
    opsegu $[\text{min},\, \text{min} + \alpha(\text{max}-\text{min})]$ -- i
    nasumično se bira jedan element RCL-a. Parametar $\alpha \in [0,1]$
...
pronađeno rešenje. Osnovni pseudokod GRASP-a dat je Algoritmom~\ref{alg:grasp}.
...
Radi preglednosti, u nastavku su objašnjeni skraćeni i kolokvijalni nazivi
koji se koriste u opisu implementacije:
\begin{itemize}
...
Ukupni trošak rešenja računa metoda \texttt{compute\_cost()} kao sumu
varijabilnih i fiksnih troškova u obe etape:
...
$y$-toka iznova rekonstruišu pohlepnom procedurom \textsc{Repair}. Ovaj
pristup smanjuje dimenzionalnost prostora pretrage jer algoritam nikad
eksplicitno ne upravlja $x$-promenljivama.
...
\begin{enumerate}
  ...
  \item \textbf{Pomeranje dela toka} (\texttt{\_shift\_flow}): dok prethodni
    ...
    dobavljači snabdevaju u celim komadima kapaciteta, pa DC čiji je protok
    tačan umnožak kapaciteta može u potpunosti osloboditi jednog
    (najskupljeg) dobavljača u I. etapi. Potez uzorkuje najviše $200$ veza,
...
uvećaju za $d$ (tzv.\ četvorougaona razmena). Ovakva razmena ne menja ni
...
\% gap-a), računate preko $k$ izvršavanja po instanci.
...
$k=10$ nezavisna izvršavanja po instanci. Na instanci~1 ($4 \times 8 \times
16$) algoritam u svih 10 izvršavanja pronalazi tačno optimalno rešenje (gap
